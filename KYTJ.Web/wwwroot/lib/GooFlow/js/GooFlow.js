...
function GooFlow(bgDiv, property) {
    ...
    if (property.haveHead) {
        ...
        for (var x = 0; x < property.headBtns.length; ++x) {
            tmp += "<a href='javascript:void(0)' class='GooFlow_head_btn'><i class='ico_" + property.headBtns[x] + "'></i>"
                + "<span>" + headbtnMark[x] + "</span> </a > "
        }
        tmp += "<div style='float:left;margin-left: 25%;width: 2px;height: 62px; background: darkgray;'></div>";
        //for (var x = 0; x < property.headToolBtns.length; ++x) {
        //    tmp += "<a href='javascript:void(0)' class='GooFlow_headtool_btn'><i class='ico_" + property.headToolBtns[x] + "'></i></a > "
        //}
        tmp += "</div>";
        this.$head = $(tmp);
        ...
        if (property.headBtns)
            this.$head.on("click", { inthis: this }, function (e) {
                ...
                switch ($(tar).attr("class")) {
                    ...
                    case "ico_remove": if (This.onRemoveClick != null) This.onRemoveClick(); break;
                }
            });
    }
    //var toolWidth = 0;
    if (property.haveTool) {
        ...
        //未加代码：加入绘图工具按钮
        this.$tool.append(
            "<a href='javascript:void(0)' type='cursor' class='GooFlow_tool_btn' style='display:none;' id='" + this.$id + "_btn_cursor'><i class='ico_cursor'/></a>"
        ...
        if (property.toolBtns && property.toolBtns.length > 0) {
            ...
            for (var i = 0; i < property.toolBtns.length; ++i) {
                if (i == 1)
                    tmp += "<a href='javascript:void(0)' type='direct' class='GooFlow_tool_btn' id='" + this.$id + "_btn_direct'><i class='ico_direct'/></a>" +
                        "<a href='javascript:void(0)' type='" + property.toolBtns[i] + "' id='" + this.$id + "_btn_" + property.toolBtns[i].split(" ")[0] + "' class='GooFlow_tool_btn'><i class='ico_" + property.toolBtns[i] + "'/></a>";
                else
    ...
    this.$workArea = $("<div class='GooFlow_work_inner' style='height:" + height*1.3 + "px'></div>")
    ...
    if (this.$editable) {
        this.$workArea.on("click", { inthis: this }, function (e) {
            ...
            This.addNode(This.$id + "_node_" + This.$max, { name: remark[This.$nowType], left: X, top: Y, type: This.$nowType });
            This.$max++;
        });
        //划线或改线时用的绑定
        ...
        this.$workArea.mouseup({ inthis: this }, function (e) {
            ...
            if (tmp) {
                ...
                if (This.$focus)
                    ...
                    var arrs = Object.keys(This.$lineData);
                    var lastLine = arrs[arrs.length - 1];
                    This.focusItem(This.$lineData[lastLine].to, false);
                }
            } else {
...
GooFlow.prototype = {
    ...
    setNodeRemarks: function (remark) {
        ...
        this.$tool.children("a").each(function () {
            //debugger
            spnTool = '<span>' + remark[$(this).attr("id").split("btn_")[1]] + '</span>';
            this.title = remark[$(this).attr("id").split("btn_")[1]];
            //this.$toolspn = $(spnTool);
            $(this).append(spnTool);
        });
    ...
    initWorkForNode: function () {
        ...
        this.$workArea.delegate(".GooFlow_item", "contextmenu", { inthis: this }, function (e) {
            ...
            var mx = e.clientX;
            var my = e.clientY;
            var type = e.data.inthis.$nodeData[this.id].type;
            //网页的宽度(高度用同样的方法解决)
            var pageWidth = document.documentElement.clientWidth;
            switch (type) {
                case "filter":
                    rm = document.getElementById("filtermenu");
                    break;
                case "source":
                    rm = document.getElementById("nodeMenu");
                    break;
                case "static":
                    rm = document.getElementById("staticMenu");
                    break;
                case "complex":
                    rm = document.getElementById("menuView");
                    break;
                case "field":
                    rm = document.getElementById("menuField");
                    break;
                case "row":
                    rm = document.getElementById("menuRow");
                    break;
                case "database":
                    rm = document.getElementById("menudata");
                    break;
                case "simpli":
                    rm = document.getElementById("menusimpli");
                    break;
                default:
                    rm = document.getElementById("menuTest");
                    break;
            }
            //菜单宽度
    ...
    focusItem: function (id, bool) {
        ...
        var filteFrom = lineValues.filter(ele => ele.to == id);
        if (filteFrom.length > 0) this.$preNode = filteFrom[0].from;
        if (this.$nodeData[id].type == 'database') {
            if (filteFrom.length > 1) this.$preNode2 = filteFrom[1].from;
        }
        this.$firstNodeId = getFirstNode(lineValues, id);
