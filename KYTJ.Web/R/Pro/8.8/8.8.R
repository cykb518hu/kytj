...
ixn7<-as.character(a[1,20]);ixv7<-as.character(a[1,21]);ixs7<-as.numeric(a[1,22])
slt.vname<-c()
...
library(nFactors,lib.loc=R.LibLocation)
...
tmp.xx<-xv[cmp,]
tmp.scree1<-nScree(x=as.data.frame(tmp.xx),model="components",criteria=0)
tmp.scree2<-nScree(x=as.data.frame(tmp.xx),model="components",criteria=1)
tmp.scree3<-nScree(x=as.data.frame(tmp.xx),model="factors",criteria=0)
tmp.scree4<-nScree(x=as.data.frame(tmp.xx),model="factors",criteria=1)
tt1<-t(rbind(names(tmp.scree1[[1]]),tmp.scree1[[1]]))
tt2<-t(rbind(names(tmp.scree2[[1]]),tmp.scree2[[1]]))
tt3<-t(rbind(names(tmp.scree3[[1]]),tmp.scree3[[1]]))
tt4<-t(rbind(names(tmp.scree4[[1]]),tmp.scree4[[1]]))
tt<-cbind(paste(tt1[,1],c("最佳坐标指数","加速因子","Parallel 分析","Kaiser 规则"),sep=": "),tt1[,2],tt2[,2],tt3[,2],tt4[,2])
tt<-rbind(c("确定因子数的方法","model=components criteria=0","model=components criteria=1","model=factors criteria=0","model=factors criteria=1"),tt)
png(paste(ofname,"_nscree1.png",sep=""),width=720,height=560)
plotnScree(tmp.scree1,main="Scree test: model=components criteria=0")
dev.off()
png(paste(ofname,"_nscree2.png",sep=""),width=720,height=560)
plotnScree(tmp.scree2,main="Scree test: model=components criteria=1")
dev.off()
png(paste(ofname,"_nscree3.png",sep=""),width=720,height=560)
plotnScree(tmp.scree3,main="Scree test: model=factors criteria=0")
dev.off()
png(paste(ofname,"_nscree4.png",sep=""),width=720,height=560)
plotnScree(tmp.scree4,main="Scree test: model=factors criteria=1")
dev.off()

w<-c("<html><head>","<meta http-equiv=\"Content-Type\" content=\"text/html\" charset=\"gb2312\" /></head><body>")
w<-c(w,paste("<h2>Scree 检验（确定因子数）</h2>",sep=""))
w<-c(w,"</br><table border=3>",mat2htmltable(tt),"</table></br>")
w<-c(w,"</br>criteria=0: λ >= \bar{λ} </br>")
w<-c(w,"</br>criteria=1: λ >= 0 (sometimes used with FA)</br>")
w<-c(w,"</body></html>")
